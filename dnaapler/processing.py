...
def process_blast_output_and_reorient(input, blast_file, out_file, ctx: click.Context, gene: str):
    ...
    # I anticipate this will be rare usually, the first will be it :)
        ...
        for i in range(0,len(blast_df.qseq)):
            if blast_df['qseq'][i][0] in ['M', 'V', 'L'] and (blast_df['sstart'][i] == 1):
                reorient_sequence(blast_df, input, out_file, gene, i)
                # warn if the top hit not used
                if i >= 1:
                    logger.warning(f"The top {gene} blastx hit was not chosen to reorient your genome, as it did not begin with a valid start codon.")
                    logger.warning(f"A lower hit had a valid start codon which was used instead.")
                gene_found = True
                break
        if gene_found == False:
